...
export default function ContasReceberPage() {
  ...
  const [clientes, setClientees] = useState<Contact[]>([])
  ...
  const loadContas = useCallback(async () => {
    ...
    const data = await getFinancialAccounts('receber', filters)
    setContas(data)

    const summaryData = await getFinancialSummary()
    setSummary(summaryData.contasReceber)

    setLoading(false)
  }, [filterStatus, filterClienteId, filterStartDate, filterEndDate])

  useEffect(() => {
    const loadData = async () => {
      const clientesData = await getContacts('cliente')
  ...
  const exportToPDF = async () => {
    ...
    doc.text('Contas a Receber', doc.internal.pageSize.getWidth() / 2, 32, { align: 'center' })
    ...
    if (filterClienteId !== 'all') {
      const cliente = clientes.find(c => c.id.toString() === filterClienteId)
      if (cliente) filterText += `Cliente: ${cliente.name} • `
    }
    ...
    doc.text(`Recebido: ${formatCurrency(summary.pago)}`, 70, 63)
    ...
    autoTable(doc, {
      startY: 72,
      head: [['Vencimento', 'Descrição', 'Cliente', 'Valor', 'Recebido', 'Saldo', 'Status']],
      body: tableData,
      foot: [[
    ...
    <div className="space-y-6">
      ...
      <Dialog open={formOpen} onOpenChange={(open) => {
        ...
        <DialogContent className="sm:max-w-[600px]">
          <DialogHeader>
            <DialogTitle>{editingConta ? 'Editar' : 'Nova'} Conta a Receber</DialogTitle>
